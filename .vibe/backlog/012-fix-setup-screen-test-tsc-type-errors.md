...
## Acceptance Criteria
- [ ] `npm run build` completes without a `tsc` error
- [ ] Every affected `vi.fn()` call in `src/setup/setup-screen.test.ts` gets an explicit type argument (e.g. `vi.fn<(config: MatchSetupConfig) => void>()`) or an equivalently-typed helper, rather than a broad cast
- [ ] `npm test` and `npm run lint` stay green
