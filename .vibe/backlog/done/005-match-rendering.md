...
## Acceptance Criteria
- [x] Both characters render at their live position with the correct current animation/sprite frame, driven by `engine` state
- [x] The selected stage's layers/background render with correct parallax/composition ordering relative to the characters (no camera panning yet, see `.vibe/decisions/004` — a documented scope cut, not a defect)
- [x] A missing or unresolvable sprite/palette reference degrades to a visible placeholder instead of a crash or a blank frame
- [x] Rendering keeps pace with `engine`'s simulation rate without visibly falling behind under normal match conditions
