...
- **`src/main.test.ts`** — the composition root's wiring, with `loadCharacter`/`loadStage` and the manifest/byte fetches all injected as fakes: the app shell mounts correctly, the roster renders from a fetched manifest, a manifest failure shows a clear message, picking both players' characters renders the stage screen, a stage manifest failure shows a clear message, picking a stage renders the match setup screen, and a full pick-both-players → pick-a-stage → configure-and-continue flow shows the final confirmation naming both players, the chosen stage, and the configured rounds/time limit.
